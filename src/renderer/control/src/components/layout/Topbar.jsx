...
export function Topbar() {
  const { theme, setTheme, isLive, clearProjection, displays } = useApp()
  const isDark = theme === 'dark'

  const secondary = displays.find(d => !d.isPrimary)
  const monitorLabel = secondary
    ? `${displays.length} monitores`
  ...
  return (
    <header className={cn(
      ...
      <h1 className="font-bold text-base text-slate-700 dark:text-slate-300">
        Panel de control
      </h1>
      ...
      <div className="ml-auto flex items-center gap-3">
        ...
        {/* Monitor badge */}
        <span className={cn(
          'font-mono text-[11px] px-2.5 py-1 rounded-full select-none',
